...
def register_subscribtion_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(unsubscribe_callback, UnsubscribeCb.filter(action=["unsubsscribe"]), state="*")
    dp.register_callback_query_handler(subscribe_callback, UnsubscribeCb.filter(action=["subsscribe"]), state="*")
    dp.register_message_handler(my_subscriptions, Text(equals="My subscriptions", ignore_case=True), state="*")
    dp.register_message_handler(unsubscribe_all, Text(equals="Unsubscribe all", ignore_case=True), state="*")

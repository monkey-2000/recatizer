...
class ClsLossAndMetricCriterion(BaseLossAndMetricCriterion):
    def __init__(self, device: torch.device, metrics: Optional[List[BaseMetric]] = None, loss="focal_loss"):
        super().__init__()
        self.loss = FocalLoss(gamma=2) if loss == 'focal_loss' else torch.nn.CrossEntropyLoss()
        self.device = device
        self.metrics = metrics or []


    def calculate(
        self, output: Dict[str, torch.Tensor], target: Dict[str, torch.Tensor], avg_meter: Dict[str, AverageMeter], training: bool
    ) -> torch.Tensor:

...
class PRMetric(BaseMetric):
    ...
    def compute(self):
        precision = np.full(shape=self.num_classes, fill_value=self.EMPTY_VAL, dtype=np.float32)
        recall = np.full(shape=self.num_classes, fill_value=self.EMPTY_VAL, dtype=np.float32)
        for i in range(self.num_classes):
            precision[i] = self.tp_classes[i] / (self.tp_classes[i] + self.fp_classes[i] + self.EPS)
            recall[i] = self.tp_classes[i] / (self.tp_classes[i] + self.fn_classes[i] + self.EPS)
            if self.tp_classes[i] == 0 and self.fn_classes[i] == 0 and self.fp_classes[i] == 0:

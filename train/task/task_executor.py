...
from train.task.callbacks.callbacks import (
    TensorBoard,
    JsonMetricSaver,
    WanDBMetricSaver,
    WanDBModelSaver,
...
class TaskRunner:
    def __init__(
        ...
        self.optimizer, self.lr_scheduler = OptimizerBuilder(optimizer_config).build(
            model
        )
    ...
    def _create_basic_callbacks(self) -> List[Callback]:
        ...
        """
        callbacks = [
            TensorBoard(self.logs_save_path, self.optimizer, self.metrics_collection),
            JsonMetricSaver(
                self.logs_save_path, self.optimizer, self.metrics_collection
            ),
            WanDBMetricSaver(self.wandb_run, self.optimizer, self.metrics_collection),
            WanDBModelSaver(self.wandb_run, self.model),
        ]
        return callbacks

    def _run_one_epoch(
        self, epoch: int, loaders: Dict[str, DataLoader], training: bool = True
    ):
        ...
        for batch_number, tasks_data in enumerate(iterator):
            ...
            iterator.set_postfix(
                {
                    "lr": float(self.lr_scheduler.get_last_lr()[-1]),
                    "epoch": epoch,
                    "mode": mode,
                    **avg_metrics,
                }
    ...
    def fit(self, loaders: Dict[str, Union[Dict[str, DataLoader], DataLoader]]):
        ...
        for epoch in range(self.start_epoch, self.nb_epoch):
            ...
            self.metrics_collection.train_metrics = self._run_one_epoch(
                epoch, loaders, training=True
            )
            self.checkpoint_handler.save_last(
                model=self.model,
                current_epoch=epoch,

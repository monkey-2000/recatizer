...
def _equal_to_byte_string(buffer: Union[bytes, Array], byte_string: bytes) -> bool:
    return (
        ...
        else all(buffer == np.frombuffer(byte_string, np.uint8))
    )
...
def has_jpeg_bytes(buffer: Union[bytes, Array]) -> bool:
    ...
    header = buffer[6:10]
    return _equal_to_byte_string(header, b"JFIF") or _equal_to_byte_string(
        header, b"Exif"
    )
...
def read_image(image_path: str) -> Array:
    ...
    try:
        image = (
            ...
            if (not use_turbo_jpeg) and (image.ndim == 3)
            else image
        )
        return image
    except:
...
def resize_image_if_needed(
    ...
    if (width, height) != (target_width, target_height):
        image = cv2.resize(
            image, dsize=(target_width, target_height), interpolation=interpolation
        )
    return image
